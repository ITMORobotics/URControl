...
from ur_control.robot import RobotModel, UniversalRobot, RobotState
...
class DualUniversalRobot(UniversalRobot):
    ...
    Args:
        ip1 (str): _description_
        ip2 (str): _description_
    ...
    def send_dq(self, dq_common):
        ...
        Args:
            dq_common (_type_): _description_
        """
...
class CoopCartState:
    ...
    def build_from_SE3(self, abs_tf: SE3, rel_tf: SE3):
        ...
        Args:
            abs_tf (SE3): _description_
            rel_tf (SE3): _description_
        """
    ...
    def build_from_matrices(self, abs_tf: np.ndarray, rel_tf: np.ndarray):
        ...
        Args:
            abs_tf (np.ndarray): _description_
            rel_tf (np.ndarray): _description_
        """
...
class DualCoopModel:
    ...
    Args:
            coop_models (Tuple[RobotModel]): _description_
    """
    ...
    def relative_jacobian(self, q_tuple: Tuple[np.ndarray]) -> np.ndarray:
        rel_jacob = np.concatenate(
            (-self.__coop_model[0].jacobian(q_tuple[0]), self.__coop_model[1].jacobian(q_tuple[1])), axis=1
        )
        return rel_jacob

    def absolute_jacobian(self, q_tuple: Tuple[np.ndarray]) -> np.ndarray:
        rel_jacob = np.concatenate(
            (-1.0 / 2 * self.__coop_model[0].jacobian(q_tuple[0]), 1.0 / 2 * self.__coop_model[1].jacobian(q_tuple[1])),
            axis=1,
        )
        return rel_jacob
...
class CoopSE3LineTrj:
    def __init__(
        self, start_coop_state: CoopCartState, finish_coop_state: CoopCartState, dt: float, finish_time: float
    ):
