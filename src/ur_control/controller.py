...
from ur_control import coop_robot
...
class CooperativeController:
    """_summary_
    Args:
            coop_model (coop_robot.DualCoopModel): _description_
            coop_stiff_matrix (np.ndarray): _description_
            coop_P_matrix (np.ndarray): _description_
            coop_I_matrix (np.ndarray): _description_
            dt (float): _description_
    """
    ...
    def world_stiff_control(
        ...
        Args:
            target_coop_rel_pose (np.ndarray): _description_
            target_coop_rel_orient (np.ndarray): _description_
            target_coop_abs_ft (np.ndarray): _description_
            q (Tuple[np.ndarray]): _description_
            ft (Tuple[np.ndarray]): _description_
            selection_matrix_T (np.ndarray): _description_
            selection_matrix_Y (np.ndarray): _description_
        ...
        if jacob_rank < abs_rel_jacob.shape[0]:
            ...
            control_dq = np.linalg.pinv(self.__coop_model.abs_rel_jacobian((q1, q2))) @ (
                target_move_twist + target_force_twist
            )
            # print("Control dq: \n", control_dq)
        return control_dq

    def world_rigid_control(
        self,
        ...
        Args:
            target_coop_abs_pose (np.ndarray): _description_
            target_coop_abs_orient (np.ndarray): _description_
            target_coop_rel_pose (np.ndarray): _description_
            target_coop_rel_orient (np.ndarray): _description_
            q (Tuple[np.ndarray]): _description_
            ft (Tuple[np.ndarray]): _description_
        ...
        abs_rot = self.__coop_model.absolute_orient((q1, q2))
        block_abs_rot = scipy.linalg.block_diag(
            np.identity(3), np.identity(3)
        )  # kroneker product for creating block diagonal matrix
        rel_rot = self.__coop_model.relative_orient((q1, q2))
        ...
        error_move_coop = np.concatenate(
            (abs_pose_error, abs_orient_twist_error, rel_pose_error, rel_orient_twist_error), axis=0
        )
    ...
    def hybride_world_control(
        ...
        Args:
            ...
            target_coop_rel_orient (np.ndarray): _description_
            target_coop_abs_ft (np.ndarray): _description_
            target_coop_rel_ft (np.ndarray): _description_
            q (Tuple[np.ndarray]): _description_
            ft (Tuple[np.ndarray]): _description_
            selection_matrix_T (np.ndarray): _description_
            selection_matrix_Y (np.ndarray): _description_
